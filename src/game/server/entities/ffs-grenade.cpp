...
CFFSGrenade::CFFSGrenade(CGameWorld *pGameWorld, int Owner, vec2 Pos, vec2 Dir)
: CEntity(pGameWorld, CGameWorld::ENTTYPE_FFS_GRENADE)
{
	m_Pos = Pos;
	m_ActualPos = Pos;
	m_ActualDir = Dir;
...
void CFFSGrenade::Reset()
{
	GameServer()->m_World.DestroyEntity(this);
}

vec2 CFFSGrenade::GetPos(float Time)
{
...
void CFFSGrenade::FillInfo(CNetObj_Projectile *pProj)
{
...
void CFFSGrenade::Snap(int SnappingClient)
{
	float Ct = (Server()->Tick()-m_StartTick)/(float)Server()->TickSpeed();
