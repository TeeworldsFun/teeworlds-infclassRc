...
{	
	...
	for(; c<sizeof(aNameFound)-1; c++)
	{
		if(pStr[c] == ' ' || pStr[c] == 0)
		{
			...
			else if(str_comp(aNameFound, "!beta") == 0 && m_apPlayers[ClientID] && m_apPlayers[ClientID]->GetCharacter())
			{
				CheckClass = PLAYERCLASS_BETA;
				str_copy(aChatTitle, "beta", sizeof(aChatTitle));
			}
